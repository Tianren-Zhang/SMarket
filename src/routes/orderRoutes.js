...
const placeOrderValidationRules = [
    ...
    body('items.*.itemId')
        .isMongoId().withMessage('Each itemId must be a valid ObjectId'),
    body('items.*.quantity')
        .isInt({min: 1}).withMessage('Each quantity must be an integer greater than 0')
];

const cancelOrderValidationRules = [
    body('customerOrder')
        .isMongoId().withMessage('Customer Order Id must be a valid ObjectId'),
    body('individualOrder').isArray().withMessage('Individual Order must be an array'),
    body('individualOrder.*')
        .isMongoId().withMessage('Each individual order Id must be a valid ObjectId')
];
...
router.get('/me/:orderId',
    authMiddleware,
    checkCustomerRole,
    body('orderId').isMongoId().withMessage('Each orderId must be a valid ObjectId'),
    validateAll,
