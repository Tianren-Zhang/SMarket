...
const updateProfileByUserId = async (userId, profileData) => {
    ...
    if (profileData && profileData.socialMedia && existingProfile && existingProfile.socialMedia) {
...
const addAddress = async (userId, addressData) => {
    ...
    const address = await userAddressRepository.createUserAddress(userId, addressData);
...
const updateAddressById = async (userId, addressId, addressData) => {
    ...
    if (!profile.addresses.map(id => id.toString()).includes(addressId)) {
        throw new NotFoundError('Address not found here', 'address', addressId, 'params');
    }
